...
- `git push --set-upstream origin <branch name>` If we dont have the current branch on remote repo, to push the current branch with all the commits and set the remote as upstream.
- `git commit --amend --author="user_name <user_email>"` while pushing the changes we may get error like author of commit is different, then to change the author details to the expected one we can use this command.
